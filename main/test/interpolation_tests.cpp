...
TEST(MeshInterpolation, CellAverageOneParticle)
{
    ...
    double totalDens = 0;
    for (size_t i = 0; i < mesh.dens_.size(); ++i) totalDens += mesh.dens_[i] * cellVolume;
    EXPECT_NEAR(totalDens, 8.0, 1e-10);

    auto [ii, jj, kk] = mesh.positionToCell(0.375, 0.375, 0.375);
    size_t idx = ii + jj * 4 + kk * 16;
    EXPECT_NEAR(mesh.dens_[idx], expectedDensity, 1e-10);
}
...
TEST(MeshInterpolation, SphOneParticleAtCenter)
{
    ...
    double cellVolume = cellSize * cellSize * cellSize;
    double totalMass = 0;
    for (size_t i = 0; i < mesh.dens_.size(); ++i) totalMass += mesh.dens_[i] * cellVolume;
    ...
    EXPECT_GT(mesh.dens_[4 + 4*8 + 4*64], 0.0);
}
